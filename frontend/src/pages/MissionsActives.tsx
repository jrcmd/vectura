...
export default function MissionsActives(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetch(`${apiBase}/driver/missions?type=active`, { headers });
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      <header className="p-4 border-b flex items-center justify-between sticky top-0 z-40 bg-white/80 backdrop-blur">
        <div className="font-extrabold">Vectura — Missions en cours</div>
      ...
      <main className="max-w-3xl mx-auto p-4 space-y-4">
        <h1 className="text-2xl font-extrabold">Missions en cours</h1>
        {loading ? (<p className="text-sm text-slate-500">Chargement…</p>) : error ? (<div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-800">{error}</div>) : missions.length === 0 ? (<p className="text-sm text-slate-600">Aucune mission en cours.</p>) : (
          <ul className="space-y-2 text-sm">
            {missions.map((m) => (
              <li key={m.id} className="rounded-xl border p-3">
                <div className="font-medium">{m.title}</div>
                <div className="text-xs text-slate-500">{m.location} • {new Date(m.missionDate).toLocaleDateString('fr-FR')} • {m.startTime}</div>
              </li>
            ))}
          </ul>
        )}
